						...
						int usb_speed)
{
	...
	if (!fpga_done) {
		...
		/* USB 2.0 (FX2) needs GPIF wordwide mode; USB 3.0 (FX3) does not. */
...
SR_PRIV int dslogic_dev_open(struct sr_dev_inst *sdi, struct sr_dev_driver *di)
{
	...
	for (i = 0; i < device_count; i++) {
		...
		sr_info("USB speed: %s.",
			devc->usb_speed == LIBUSB_SPEED_SUPER ? "SuperSpeed (USB 3.0)" :
			devc->usb_speed == LIBUSB_SPEED_HIGH ? "High-Speed (USB 2.0)" :
			"unknown");
